...
if (process.env.NODE_APP_INSTANCE === "0") {
    cron.schedule('*/1 * * * *', async () => {
        // 선거 시작 제어
        try {
            let [election] = await pool.query('SELECT id, name, admin_id, flag, UNIX_TIMESTAMP(start_dt) as start_dt FROM  election')
    ...
    cron.schedule('*/1 * * * *', async () => {
        // 선거 종료 제어
        try {
            let [election] = await pool.query('SELECT id, name, flag,  admin_id,  UNIX_TIMESTAMP(end_dt) as end_dt FROM election')
            await election.map(async data => {
                if (data.flag == 1) {
